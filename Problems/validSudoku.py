...
def isValidSudoku2(board: List[List[str]]) -> bool:
    ...
    for i in range(0,9):
        for j in range(0,9):
            character = board[i][j]
            if (character != '.'):
                tuple_tracker.append((i, character))
                tuple_tracker.append((character, j))
                tuple_tracker.append((i//3, j//3, character))

    return len(tuple_tracker) == len(set(tuple_tracker))
